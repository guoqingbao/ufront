...
impl Operator {
    pub fn new(op : OpType, params : HashMap<String, String>) -> Operator {
        ...
        // })
        Operator {inputs : Vec::new(), outputs :Vec::new(), op_type : op, params : params.clone()}
    ...
    pub fn calculate_output(&mut self) {
        ...
        match self.op_type {
                ...
                && self.params.contains_key("stride_w") {
                    ...
                    match &self.inputs[0].tensor {
                        Some(v) => {
                            ...
                            let output_channel = if self.params.contains_key("out_channels") { self.params["out_channels"].trim().parse::<usize>().unwrap() } else { v.shape[1] };
            ...
            OpType::EXP | OpType::ADD | OpType::SUBTRACT | OpType::DIVIDE 
            | OpType::MULTIPLY | OpType::DROPOUT | OpType::ELU | OpType::RELU | OpType::GELU | OpType::SIGMOID | OpType::TANH |
            OpType::SCALAR_ADD | OpType::SCALAR_FLOORDIV | OpType::SCALAR_MULTIPLY | OpType::SCALAR_SUB | OpType::SCALAR_TRUEDIV => {
                ...
                match &self.inputs[0].tensor {
                    Some(v) => {
                        println!("Output tensor with shape {:?} created within Rust for operator {:?}!", v.shape, self.op_type);
                        let tensor = Tensor::<f32> {
                            shape : v.shape.clone(), 
                            data_buffer :  DataBuffer::CPUDataBuffer(vec![0f32; v.shape.iter().product()]),
                        };
                        self.add_output(tensor);
            ...
            OpType::CONCAT => {
                ...
                match &mut self.inputs[0].tensor {
                    Some(v) => { output_shape = v.shape.clone();}
                    _ => {}
                }
                ...
                for tensor in &self.inputs {
                    match &tensor.tensor {
                        ...
                        _=> {panic!("Invalid tensor for axis {} concat!", self.params["axis"]);}
                    }
                }
            ...
            OpType::SPLIT => {
                ...
                match &mut self.inputs[0].tensor {
                    Some(v) => {
                        let sizes = if self.params.contains_key("sizes") {self.params["sizes"].trim().parse::<usize>().unwrap()} else {1};
                        let idx = self.params["axis"].trim().parse::<usize>().unwrap();
                    ...
                    _=> {panic!("Invalid tensor for axis {} split!", self.params["axis"]);}
                }
            ...
            OpType::SOFTMAX | OpType::FLAT => {
                ...
                match &mut self.inputs[0].tensor {
                    Some(v) => {
                        ...
                        self.add_output(tensor);
                        println!("Output tensor with shape {:?} created within Rust for operator {:?}!", vec![sz], self.op_type);
                    }
                    _=> {panic!("Invalid tensor for flat!");}
                }
            ...
            OpType::LINEAR => {
                ...
                self.add_output(tensor);
                println!("Output tensor with shape {:?} created within Rust for operator {:?}!", vec![output_dim], self.op_type);
            }
            _ => {
...
impl PyOperator {
    #[new]
    pub fn new(op_type : OpType, params : HashMap<String, String>) -> PyResult<PyClassInitializer<Self>> {
        println!("PyOperator::new");
        let op = PyOperator { op_type : op_type, params : params, raw_ptr : 0};
    ...
    pub fn add_input_ndarray(&mut self,
        ...
        match x.as_slice() {
            Some(v) => {
                ...
                else {println!("Input not added to the graph!");}  
    ...
    ) -> PyResult<()>
    {
        ...
        else {panic!("Input not added to the graph!");}  
    }
    ...
    pub fn get_output(&self, idx : usize) -> Py<TensorF32>  {
                    ...
                    Ok(tensor) => {
                        match &tensor.tensor {
                            Some(v) => {
                                ...
                                Python::with_gil(|py| Py::new(py, TensorF32 {tensor : Some(tensor) }).unwrap())
                            }
                            _=> {panic!("Unable to obtain output!");}
                        }
        ...
        else { panic!("Invalid operator pointer!");}   
    }
